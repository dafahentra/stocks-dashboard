...
from styles import apply_minimal_style, COLORS
...
# Main dashboard
if st.sidebar.button("Update Dashboard", type="primary", use_container_width=True) or auto_refresh:
            ...
            if last_close is not None:
                ...
                with tab3:
                    ...
                    with col1:
                        ...
                        price_df = data[['Datetime', 'Open', 'High', 'Low', 'Close', 'Volume']].tail(10)
...
with st.sidebar:
    ...
    watchlist = {
        ...
        'European': ['SAP.DE', 'ASML.AS'],
        'Asian': ['TSM', '7203.T'],
        'Indonesian': ['GOTO.JK', 'BBCA.JK', 'TLKM.JK']
    }
            ...
            for symbol in symbols:
                try:
                    ...
                    if not quick_data.empty:
                        ...
                        st.metric(symbol, f"{curr_price:.2f}", f"{pct:.2f}%", delta_color="normal")
                except:
                    st.metric(symbol, "N/A", "Error")

    st.markdown("---")
    st.caption("Data: Yahoo Finance | © 2025")