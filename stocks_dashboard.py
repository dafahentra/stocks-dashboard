...
st.markdown("*Comprehensive Stocks Report & Trend Analytics*")

# Sidebar controls
with st.sidebar:
    st.markdown("### Dashboard Settings")
    
    ticker = st.text_input("Stock Ticker", "AAPL", help="e.g., AAPL, GOOGL, BMW.DE").upper()
    ...
    with st.spinner(f"Loading {ticker} data..."):
        ...
        if not data.empty:
            ...
            if last_close is not None:
                ...
                with col1:
                    st.metric("Current Price", format_price(last_close, currency), 
                ...
                with tab3:
                    ...
                    with col2:
                        ...
                        # Price trend
                        if pct_change > 0:
                            styled_metric(f"Bullish: +{pct_change:.2f}%", "#144D14", "#39FF14")
                        else:
                            styled_metric(f"Bearish: {pct_change:.2f}%", "#5B1414", "#FF4040")

                        # RSI analysis
                        if 'RSI' in data.columns and not data['RSI'].isna().iloc[-1]:
                            rsi_val = data['RSI'].iloc[-1]
                            if rsi_val > 70:
                                styled_metric(f"RSI: {rsi_val:.1f} (Overbought)", "#663300", "#FFB84D")
                            elif rsi_val < 30:
                                styled_metric(f"RSI: {rsi_val:.1f} (Oversold)", "#003366", "#66CCFF")
                            else:
                                styled_metric(f"RSI: {rsi_val:.1f} (Neutral)", "#333333", "#CCCCCC")
                        ...
                        if 'SMA_20' in data.columns and 'SMA_50' in data.columns:
                            ...
                            if sma20 > sma50:
                                styled_metric("MA Signal: Bullish", "#144D14", "#39FF14")
                            else:
                                styled_metric("MA Signal: Bearish", "#5B1414", "#FF4040")

                        # Volume analysis
                        avg_vol = data['Volume'].mean()
                        curr_vol = data['Volume'].iloc[-1]
                        if curr_vol > avg_vol * 1.5:
                            styled_metric("Volume: High", "#003366", "#66CCFF")
                        elif curr_vol < avg_vol * 0.5:
                            styled_metric("Volume: Low", "#663300", "#FFB84D")
                        else:
                            styled_metric("Volume: Normal", "#333333", "#CCCCCC")
            ...
            for symbol in symbols:
                try:
                    ...
                    if not quick_data.empty:
                        ...
                        st.metric(symbol, f"{curr_price:.2f}", f"{change:.2f} ({pct:.2f}%)")
                except:
                    st.metric(symbol, "N/A", "Error")

    st.markdown("---")
    st.caption("Data: Yahoo Finance | Real-time updates available")